...
async function test() {
  try {
    ...
    try {
      console.log("Fetching exact farm...");
      const fazendaId = "fazenda_" + uid;
      const f = await getDoc(doc(db, 'fazendas', fazendaId));
      if (f.exists()) {
        console.log("Farm found:", f.data().nome);
      } else {
        console.log("Farm not found");
      }
    } catch (e) {
      console.error("Error fetching farm:", e.message);
    }
